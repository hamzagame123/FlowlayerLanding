const chapters = [
  ...
  },
  {
    ...
    body: "Built on Mapbox GL — a geospatial 3D engine that renders real cities at 1:1 scale using live map tiles and OpenStreetMap building data. Google Routes API computes the raw route, then Vertex AI augments it with context — street history, landmarks, what's around the next corner.",
    vibe: "READY",
    cards: [
      { label: "3D ENGINE", val: "MAPBOX GL" },
      { label: "BUILDINGS", val: "MAPBOX 3D" },
      { label: "TERRAIN", val: "MAPBOX STANDARD" },
    ]
  },
  {
    kicker: "THE ROUTE",
    title: 'Real Roads, <em>Real Cities</em>',
    body: "Google Routes API computes the raw route — real turn-by-turn directions on real roads. That route data is then augmented by Vertex AI, which layers in context about what you're passing, where you are, and what makes each stretch interesting. A Leaflet mini-map tracks your position live.",
    vibe: "ANALYTICAL",
    cards: [
      { label: "ROUTING", val: "GOOGLE API" },
      { label: "ROAD DATA", val: "OSM EXTRACT" },
      { label: "AI NAV", val: "VERTEX AI" },
    ]
  },
  {
...
function pickChip(btn) {
  ...
  selectedAnswers[promptIdx] = btn.textContent.trim();
  persistOutsideProfile();
...
function submitAnswer(text) {
  ...
  selectedAnswers[promptIdx] = answer;
  persistOutsideProfile();
  dom.transcript.textContent = `"${answer}"`;
...
function inferVibeFromAnswers() {
  const text = selectedAnswers.join(" ").toLowerCase();
  if (/(fast|quick|direct|highway|efficient|sharp|alert|focused)/.test(text)) return "fastest";
  if (/(calm|easy|quiet|slow|gentle|peaceful|chill|relaxed|wide)/.test(text)) return "chill";
  if (/(strange|adventure|surprise|neon|alive|energy|rush|city)/.test(text)) return "adventure";
  return "scenic";
}

function routeForVibe(vibe) {
  return {
    fastest: "coastal",
    scenic: "coastal",
    chill: "forest",
...
function persistOutsideProfile(name = "") {
  const vibe = inferVibeFromAnswers();
  localStorage.setItem("flowlayer_outside_vibe", vibe);
  localStorage.setItem("flowlayer_outside_route", routeForVibe(vibe));
  localStorage.setItem("flowlayer_outside_destination", "CN Tower");
  if (name) localStorage.setItem("flowlayer_outside_name", name);
}
