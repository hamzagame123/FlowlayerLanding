...
export class NeedleTilesRuntime {
    ...
    init() {
        ...
        this.tilesRenderer.maxDepth = 18;
    ...
    _styleTileMaterials() {
        ...
        this.tilesRenderer.group.traverse(child => {
            if (!child.isMesh || !child.material || child.userData.flowlayerStyled) return;
            const materials = Array.isArray(child.material) ? child.material : [child.material];
            const styled = materials.map(material => {
                const clone = material.clone();
                if ("color" in clone && clone.color) clone.color.setHex(0x8d94a3);
                if ("emissive" in clone && clone.emissive) clone.emissive.setHex(0x111827);
                ...
                if ("roughness" in clone) clone.roughness = 0.82;
                if ("metalness" in clone) clone.metalness = 0.03;
                if ("depthWrite" in clone) clone.depthWrite = true;
                if ("depthTest" in clone) clone.depthTest = true;
                return clone;
            });
            child.material = Array.isArray(child.material) ? styled : styled[0];
            child.userData.flowlayerStyled = true;
        });
    }
     ...
     */
    snapToSurface(position, surfaceOffset = 1.0, maxRise = 2.5) {
        ...
        const candidates = hits
            .filter(entry => entry.point && Number.isFinite(entry.point.y))
            .sort((a, b) => a.point.y - b.point.y);
        const hit = candidates[0];
        if (!hit) return null;
        if (hit.point.y - position.y > maxRise) return null;
